...
Clone 好项目后，把下面这段话发给你的 Agent，它就能接管：

> 我在 `{项目路径}` 下有一个个人理解与服务系统。先读项目根目录的 `SKILL.md`，里面有完整的命令手册。第一步跑 `python3 src/services/data_sources.py --discover` 看有哪些数据源，然后 `python3 src/services/pipeline.py --dry-run` 验证管线。

Agent 拿到 SKILL.md 后可以：
