...
> 一个跑在 [OpenClaw](https://github.com/openclaw/openclaw) 上的 Agent Skill：从多源数据理解用户，主动提供服务，追踪反馈，自己养自己。
...
`setup.sh` 会自动完成：创建运行时数据目录、安装 Python 依赖、生成配置模板。
...
```bash
# 编辑配置文件（setup.sh 已从 config.example.yaml 生成）
vim config.yaml
```
