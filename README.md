...
把下面这段话发给你的 Agent，它会自己完成所有事情：

> 帮我把这个项目 clone 下来并跑起来：`git clone git@github.com:Jocky-star/prism-workspace.git`。clone 完之后读项目里的 `SKILL.md`，按里面的步骤配置 LLM 并验证管线能跑通。

Agent 会自动执行：
1. `git clone` 到当前工作目录
2. 读 `SKILL.md` 获取完整操作手册
3. 配置 LLM（设置 `LLM_BASE_URL` + `LLM_API_KEY`）
4. `python3 src/services/data_sources.py --discover` 发现可用数据源
5. `python3 src/services/pipeline.py --dry-run` 验证管线

之后 Agent 就拥有了所有能力：跑服务管线、意图追踪、设备偏好控制、用户画像查询等。
