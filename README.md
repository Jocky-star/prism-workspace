...
把这段话发给你的 AI Agent（OpenClaw / Claude / ChatGPT / 任何能执行命令的 Agent）：

> 我有一个基于多数据源的个人理解与服务系统，代码在 `~/.openclaw/workspace/`。先读项目根目录的 `SKILL.md`，里面有完整的命令手册。读完后你就知道怎么操作了。第一步先跑 `python3 src/services/data_sources.py --discover` 看看有哪些可用数据源，然后 `python3 src/services/pipeline.py --dry-run` 验证管线是否通。

### OpenClaw 用户

如果你用 OpenClaw，可以直接作为 skill 加载：

```bash
# 方式一：clone 到 skills 目录（自动发现）
cd ~/.openclaw/workspace/skills/
git clone git@github.com:Jocky-star/prism-workspace.git prism

# 方式二：在 AGENTS.md 中注册
echo "- prism: ~/.openclaw/workspace/SKILL.md" >> AGENTS.md
```

Agent 启动后会自动读取 SKILL.md，获得所有操作能力。

### 其他 Agent 框架

把 `SKILL.md` 内容注入 system prompt 或上下文即可：
...
messages = [
    {"role": "system", "content": f"你可以使用以下命令操作系统：\n\n{skill}"},
    {"role": "user", "content": "帮我跑一次今天的服务管线"}
]
```
...
- 🎯 跑服务管线（晨间 Brief / 意图追踪 / 情绪关怀 / 人际洞察）
- 🧠 运行智能理解（感知 → 理解 → 精炼）
- 💡 管理设备偏好（用户说一句话 → 台灯行为自动改变）
- 📊 查询用户画像和行为模式
- 📺 控制 Prism 屏幕（需硬件）
- 🔌 接入新数据源、扩展新服务
