...
│
├── prompts/          # 📝 Prompt 模板层 — 可 git 管理的 prompt 文件
│   ├── nightly_execution.md  # 每晚自主执行 prompt（被 cron 引用）
│   └── morning_brief.md      # 晨间 Brief system prompt（被 daily_brief.py 引用）
│
├── templates/        # 📋 初始化模板 — 新用户 setup 时从这里复制
│   └── service_pool.example.json  # 服务池示例（6 个默认服务，带字段注释）
│
...
| Prompt 模板 | `src/prompts/` | .md 文件，可 git 管理 |
| 初始化模板 | `src/templates/` | 新用户 setup 用的示例配置 |
...
`memory/action_log/YYYY-MM-DD.jsonl` — 每行一条 JSON：

```jsonc
{
  "ts": "2026-03-17T01:00:00+08:00",   // ISO 8601 时间戳
  "category": "proactive",              // proactive / delivery / system
  "title": "查了北京→福州机票",          // ≤20字，出现在 Brief 里
  "detail": "清明4/4厦航¥500最便宜",     // 具体结果，必须有数据
  "source": "learning",                 // 来源：learning / cron / manual
  "topic_id": "fuzhou-flight",          // 关联 service_pool.service_id（可选）
  "narrative_type": "tracking"          // first_discovery / tracking / update
}
```

`narrative_type` 字段控制 Brief 的叙述方式：
- `first_discovery` — 第一次做这件事，Brief 里写「新发现：...」
- `tracking` — 跟踪更新，写「更新：上次¥590，现在¥500」
- `update` — 信息更新，写「变化：...」

### 服务池机制

```
src/templates/service_pool.example.json  ← 入 git，新用户模板
       ↓ main.py setup 时复制
memory/service_pool.json                 ← 不入 git，个人化配置
       ↓ 每晚 Agent 执行时读取
nightly_execution prompt                 ← 指导 Agent 按优先级选服务执行
       ↓ 执行完写入
memory/action_log/YYYY-MM-DD.jsonl       ← Brief 的事实数据源
```

**冷却期逻辑**：`last_executed + cooldown_days * 86400 > now` 时跳过该服务。
**优先级选择**：取所有 `status=active` 且未过冷却期的服务，按 `priority` 降序排列，依次执行。

### Brief 防重复逻辑

`memory/services/YYYY-MM-DD.json` — 每天的 Brief 输出缓存。
每次推送前检查：
- 如果今天已有缓存且 `sent=true`，跳过推送（避免重复发飞书）
- `--force` 参数强制重新生成和推送
...
```
src/prompts/
├── nightly_execution.md   ← 每晚自主执行 prompt
│                            被 cron-setup 读取、输出给用户配置 OpenClaw cron
└── morning_brief.md       ← 晨间 Brief system prompt
                             被 daily_brief.py 运行时读取
                             修改文件 = 修改 Brief 生成逻辑，无需改代码
```

**加载顺序**（daily_brief.py）：
1. 尝试读取 `src/prompts/morning_brief.md`
2. 文件不存在时回退到代码内联 prompt（向后兼容）
