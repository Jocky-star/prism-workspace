...
def _load_prompt_template(name: str) -> str:
    ...
    while _src.name != "src" and _src != _src.parent:
        _src = _src.parent
    if _src.name == "src":
        prompt_path = _src / "prompts" / name
    else:
        prompt_path = Path(__file__).resolve().parent.parent.parent / "src" / "prompts" / name
...
def _build_action_log_summary(data: Dict[str, Any]) -> str:
    ...
    if yesterday_actions:
        lines = [f"  [{a.get('category','?')}] {a.get('title','')} — {a.get('detail','')[:500]}" for a in yesterday_actions[-20:]]
        parts.append("【昨日行动】\n" + "\n".join(lines))

    if today_actions:
        lines = [f"  [{a.get('category','?')}] {a.get('title','')} — {a.get('detail','')[:500]}" for a in today_actions[-10:]]
        parts.append("【今日早间行动】\n" + "\n".join(lines))
...
def generate_brief(date: str, dry_run: bool = False) -> Dict[str, Any]:
    ...
    # 从 src/prompts/morning_brief.md 加载 prompt 模板
    _prompt_tpl = _load_prompt_template("morning_brief.md")
    if _prompt_tpl:
        system_prompt = _prompt_tpl
    else:
        # 向后兼容：文件不存在时回退到内联 prompt
        system_prompt = """你是私人秘书。基于对老板的录音、对话、行为数据，生成今日简报。
...
## 输出 JSON
{
  "key_conclusions": ["..."],
  "minor_updates": ["..."],
  "decisions_needed": [{"item": "...", "option_a": "...", "option_b": "..."}],
  "system_status": "一切正常",
  "proactive": [{"insight": "...", "action": "...", "result": "..."}],
  "deliveries": [{"title": "...", "detail": "..."}]
}

⚠️ 宁可只有1条 proactive，也不要用系统改进凑数。"""
...
def format_brief_message(brief: Dict[str, Any]) -> str:
    """Format brief into a secretary-style message with expanded proactive details.

    结构（v5）：
      ☀️ 早安 Brief | M月D日
      💡 帮你做了这些（proactive 完整展开，每条100-200字）
      🔴 重要（非 proactive 的 key_conclusions）
      🎯 需要你选（A/B 决策项）
      🔵 跟进（minor_updates）
      📊 系统状态（一行）
    """
    ...
    if proactive_items:
        ...
        for p_item in proactive_items:
            ...
            scene = p_item.get("scene", "")

            # 小标题行：优先用 action，其次 insight 截短
            if action:
                # 去掉 action 里过长的前缀，取精华部分（≤30字）
                title = action[:35].rstrip("，。…") + ("..." if len(action) > 35 else "")
                parts.append(f"**{title}**")
            elif insight:
                title = insight[:35].rstrip("，。…")
                parts.append(f"**{title}**")
            ...
            # insight（如果不在 result 里，补充说明）
            if insight and result_str and insight not in result_str and action and insight not in action:
                parts.append(f"_→ {insight}_")
            elif insight and not result_str and not action:
                parts.append(insight)
    ...
    # 过滤掉已经在 proactive 里展示过的内容，以及技术类内容
    ...
    # 去掉和 proactive result 重复的
    proactive_result_snippets = [p.get("result", "")[:20] for p in proactive_items if p.get("result")]
    non_tech_conclusions = [
        c for c in non_tech_conclusions
        if not any(snip in c for snip in proactive_result_snippets if snip)
    ]

    if non_tech_conclusions:
        parts.append("**🔴 重要**\n")
        for c in non_tech_conclusions[:3]:
            parts.append(f"- {c}")
        parts.append("")
    ...
    if decisions:
        ...
        for d in decisions:
            ...
            ob = d.get("option_b", "")
            if oa and ob:
                parts.append(f"- **{item}**：A) {oa}  vs  B) {ob}")
            elif oa:
    ...
    minor_updates = tech_conclusions + minor_updates
    if not minor_updates:
        ...
        for i in b.get("captured_intents", []):
            if i.get("status") == "done":
                ...
                if quote and action_taken:
                    minor_updates.append(f"{quote} → {action_taken}")

    minor_updates = [m for m in minor_updates if not any(w in m for w in _VAGUE_WORDS)]

    if minor_updates:
        parts.append("**🔵 跟进**\n")
        for m in minor_updates[:5]:
            parts.append(f"- {m}")
        parts.append("")
...
def mark_action_log_displayed(date: str) -> int:
    """将指定日期 action_log jsonl 中所有未标记的条目标记为 displayed: true。

    实现：读取全部行 → 对未展示条目设置 displayed=True → 重写文件。
    返回标记的条目数。
    """
    from src.services.config import MEMORY_DIR as _MEMORY_DIR
    log_file = _MEMORY_DIR / "action_log" / f"{date}.jsonl"
    if not log_file.exists():
        return 0

    lines_raw = log_file.read_text(encoding="utf-8").strip().split("\n")
